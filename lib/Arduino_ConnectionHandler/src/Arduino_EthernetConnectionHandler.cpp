...
EthernetConnectionHandler::EthernetConnectionHandler(
    const char* ip, const char* dns, const char* gateway, const char* netmask, bool const keep_alive
)
    : ConnectionHandler{keep_alive, NetworkAdapter::ETHERNET},
      _ip{INADDR_NONE},
      _dns{INADDR_NONE},
...
NetworkConnectionState EthernetConnectionHandler::update_handleConnecting() {
    if (_ip != INADDR_NONE) {
        ...
        if (Ethernet.begin(nullptr, 15000, 4000) == 0) {
            Debug.print(
                DBG_ERROR,
                F("Waiting Ethernet configuration from DHCP server, check cable connection")
            );
            return NetworkConnectionState::CONNECTING;
        }
    }
