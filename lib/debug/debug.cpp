...
void Arduino_DebugUtils::setDebugLevel(int const debug_level) { _debug_level = debug_level; }

int Arduino_DebugUtils::getDebugLevel() const { return _debug_level; }

void Arduino_DebugUtils::setDebugOutputStream(Stream* stream) { _debug_output_stream = stream; }

void Arduino_DebugUtils::newlineOn() { _newline_on = true; }

void Arduino_DebugUtils::newlineOff() { _newline_on = false; }

void Arduino_DebugUtils::debugLabelOn() { _print_debug_label = true; }

void Arduino_DebugUtils::debugLabelOff() { _print_debug_label = false; }

void Arduino_DebugUtils::timestampOn() { _timestamp_on = true; }

void Arduino_DebugUtils::timestampOff() { _timestamp_on = false; }

void Arduino_DebugUtils::print(int const debug_level, const char* fmt, ...) {
    if (!shouldPrint(debug_level)) return;

    if (_print_debug_label) printDebugLabel(debug_level);

    if (_timestamp_on) printTimestamp();
...
void Arduino_DebugUtils::printTimestamp() {
    ...
    uint16_t hours = time->hour();
    uint16_t minutes = time->minute();
    uint16_t seconds = time->second();

    snprintf(
        timestamp, sizeof(timestamp), // "prints" formatted output to a char array (string)
        "[ "
        "%02d:" // HH:
        "%02d:" // MM:
        "%02d"  // SS.
...
void Arduino_DebugUtils::printDebugLabel(int const debug_level) {
    static char const* DEBUG_MODE_STRING[5] = {
        "[DBG_ERROR  ] ", "[DBG_WARNING] ", "[DBG_INFO   ] ", "[DBG_DEBUG  ] ", "[DBG_VERBOSE] ",
    };

    bool is_valid_debug_level = (debug_level >= DBG_ERROR) && (debug_level <= DBG_VERBOSE);
    if (!is_valid_debug_level) return;
...
bool Arduino_DebugUtils::shouldPrint(int const debug_level) const {
    return (
        (debug_level >= DBG_ERROR) && (debug_level <= DBG_VERBOSE) && (debug_level <= _debug_level)
    );
}
...
int getDebugMessageLevel() { return Debug.getDebugLevel(); }
