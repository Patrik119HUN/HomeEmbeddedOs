...
std::map<string, Command> commands{{"cd", {cdCallback, true}},        {"ls", {lsCallback, false}},           {"mkdir", {mkdirCallback, true}},
                                   {"rmdir", {rmdirCallback, true}},  {"restart", {restartCallback, false}}, {"pwd", {pwdCallback, false}},
                                   {"i2c_scan", {i2c_scanner, false}}};
...
void shell(void *) {
    while (true) {
        ...
        if (read != 0) {
            ...
            auto arg = commands[cmd].hasArg ? static_cast<void *>(&texts.at(1)) : NULL;
